...
function renderShoppingList() {
  ...
  if (STORE.searchBy !== ""){
    filteredItems = filteredItems.filter(item => item.name.includes(STORE.searchBy));
  }
...
function editListItem(itemId) {
  console.log(`Editing item with id  ${itemId} from shopping list`)
  const itemSelected = STORE.items.find(item => item.id === itemId);
  const itemName = prompt("Enter the fruit name", itemSelected.name);
  if (itemName != null) {
...
function toggleSearchFilter(searchBy) {
...
function handleSearchItems() {
  $('.js-item-search').on('click', () => {
    toggleSearchFilter($('#search-entry').val());
    renderShoppingList();
  });
}

function handleClearSearchItems() {
  $('.js-item-clear').on('click', () => {
    $('#search-entry').val("");
    toggleSearchFilter("");
    renderShoppingList();
  });
